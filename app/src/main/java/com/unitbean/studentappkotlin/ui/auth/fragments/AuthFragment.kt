...
class AuthFragment : MvpAppCompatFragment(), AuthView, View.OnClickListener  {
    ...
    override fun onLogout() {
        //if (requireActivity() is MainActivity) Snackbar.make(cl_auth_fragment, "Все поля должны быть заполнены", Snackbar.LENGTH_SHORT).show()
        ...
        when (v) {
            b_auth_accept -> {
                if (et_auth_first_name.text.toString() == "" || et_auth_last_name.text.toString() == "" || et_auth_institute.text.toString() == "" || et_auth_course.text.toString() == "" || et_auth_group.text.toString() == "" || et_auth_recordBook.text.toString() == "") {
                    ...
                    presenter.authUser(UserModel("19", et_auth_first_name.text.toString(), et_auth_last_name.text.toString(), et_auth_institute.text.toString(), et_auth_course.text.toString(), et_auth_group.text.toString(), et_auth_recordBook.text.toString()))
